...
そのために、ソースコードをマシン語ではなく、**バイトコード**という中間表現に変換します。このバイトコードをIgnitionが解釈しながら実行することで、重いコンパイル作業を待つことなく、すぐにコードが動き出すのです。
-->
...
ただし、オブジェクトを作った**後から**プロパティを追加・削除すると、そのたびに新しいHidden Classが作られ、最適化の妨げになります。
-->
...
V8は可能な限り特化度の高い Elements Kind を維持しようとしますが、型の異なる要素が追加されると、より一般的な種類へと基本的に一方向の遷移（降格）が発生します（同じ配列は原則戻りません。新しい配列を生成すれば `PACKED` にできます）。なお、2025年時点では `Array.prototype.fill` に限り、穴埋め時に `HOLEY` から `PACKED` へ戻す最適化が入った環境があります。
...
<!--
Maglevの強みは、Ignitionからの型情報を活用することにあります。
「この変数はいつも数値だった」という情報があれば、それを信じて数値専用の効率的なコードを生成します。
また、小さな関数は、呼び出し元にその中身を展開する**インライン展開**を行い、関数呼び出しのオーバーヘッドを削減します。
Maglevは、やりすぎない範囲で、賢く最適化を行う現実主義者なのです。
-->
