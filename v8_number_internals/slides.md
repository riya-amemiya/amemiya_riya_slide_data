...
<div class="w-full">
  <div class="eyebrow">V8 Internals / JavaScript Number</div>
  <h1 class="mega">
...
<div class="w-full text-center">
  ...
  <h1 style="color:#243762;">
    ...
    V8の中では<br>
    <span class="blue">別の姿</span>で扱われることを知る
  </h1>
  <p class="lede" style="margin-top:36px; text-align:center;">…そして、なぜそうするのかを理解する。</p>
</div>
...
TypeScriptで見ても、どれも <strong>number</strong>。<br>
...
<div class="prompt"><span class="ask">Q.</span> これは何が表示されるでしょう？</div>
...
<div class="result-bar">
  <div class="b ok">42 も number</div>
  <div class="b ok">4.2 も number</div>
  <div class="b ok">NaN も number</div>
</div>

<div class="callout" style="margin-top:36px">
JavaScriptから見える世界では、これ以上の分類は基本的にありません。
</div>
...
<div class="number-split">
  <div class="number-card">
    ...
    <div class="desc">小さな整数。<br>値そのものをタグ付き値に埋め込む。</div>
  </div>
  <div class="number-card">
    <div class="label">4.2</div>
    <div class="type">HeapNumber</div>
    <div class="desc">double値。<br>ヒープ上のオブジェクトとして持つ。</div>
  </div>
</div>

<p class="caption" style="margin-top:28px; text-align:center;">JavaScript上はどちらも <code>number</code>。でも内部表現は違う。</p>
...
<!--
では、その違いを理解するために、V8のタグ付き値の話をします。
-->
...
<p class="lede">
V8はJavaScriptの値を、内部的には <strong class="blue">Tagged Value</strong> として扱います。
</p>
...
末尾のビットをタグとして使い、<br>
「これは小さな整数なのか」「ヒープ上のオブジェクトへの参照なのか」を区別します。
</div>

<!--
V8では値をタグ付きの値として扱います。
ざっくり言うと、末尾のビットをタグとして使い、これはSmiなのか、ヒープオブジェクトへの参照なのかを区別します。
実際のビット幅や圧縮の有無は環境によりますが、考え方としては「値にタグを埋め込む」です。
-->
...
<div class="callout" style="margin-top:32px">
整数のたびにNumberオブジェクトを作ると高コスト。<br>
よく出る小さな整数は、タグ付き値の中に直接埋め込みます。
</div>

<!--
SmiはSmall Integerの略です。
ループカウンタや配列のindexなど、整数はJavaScriptの実行中に大量に出てきます。
そのたびにヒープにNumber用のオブジェクトを作っていたら、メモリもGCも大変です。
なので、小さな整数は値そのものをタグ付き値に埋め込んで扱います。
-->
...
<div class="callout" style="margin-top:36px">
小数、NaN、Infinity、Smiに収まらない整数などは、<br>
HeapNumberとして表現されます。
</div>
...
<div class="two-col wide">
  <div>
    <div class="col-label">Smiにできると嬉しい</div>
    <ul class="clean">
      <li>ヒープ割り当てがいらない</li>
      <li>GC対象が増えない</li>
      <li>整数演算として扱いやすい</li>
  ...
  <div>
    <div class="col-label">HeapNumberが必要な理由</div>
    <ul class="clean">
      <li>小数を表現できる</li>
      <li>NaN / Infinity / -0 を表現できる</li>
      <li>Number仕様の広い範囲をカバーできる</li>
    </ul>
  </div>
</div>

<div class="callout" style="margin-top:36px">
つまり、<strong>速く・小さく扱える値はSmi</strong>、<br>
それでは足りない値は <strong>HeapNumber</strong> へ。
</div>

<!--
なぜ分けるのかというと、Smiにできる値は圧倒的に扱いやすいからです。
ヒープ割り当てが不要で、GC対象も増えません。
一方で、JavaScriptのNumberは小数やNaN、Infinity、-0なども表現する必要があります。
そのため、Smiで扱えるものはSmi、それ以外はHeapNumberという分担になります。
-->
...
<!--
ここから配列の話に入ります。Number単体でも表現が分かれますが、配列に入るとさらに面白くなります。
-->
...
<div class="callout" style="margin-top:28px">
JavaScriptから見ると、どれもただの <code>Array</code>。<br>
でもV8は、配列の要素に応じて <strong class="blue">Elements Kind</strong> を変えます。
</div>

<!--
V8は配列の中身も見ています。
JavaScriptから見るとどれもArrayですが、中身が小さな整数だけなのか、小数を含むのか、文字列など任意の値を含むのかでElements Kindが変わります。
-->
...
<div class="callout" style="margin-top:32px">
小さな整数だけなら、要素はSmiとして扱える。<br>
ヒープ上のNumberオブジェクトを要素ごとに作らなくてよい。
</div>
...
<div class="callout" style="margin-top:32px">
小数が混ざると、Smi配列からdouble配列へ。<br>
この場合、配列の backing store にはdouble値を直接並べられます。
</div>

<!--
そこに4.2のような小数をpushすると、PACKED_DOUBLE_ELEMENTSへ遷移します。
ここで面白いのは、配列内ではHeapNumberへの参照を並べるのではなく、double値を直接並べる表現が使われることです。
だから「小数は常にHeapNumber」と言い切ると少し雑で、配列の中ではdoubleの生値として持てるケースがあります。
-->
...
<div class="callout" style="margin-top:32px">
任意のJS値が混ざると、より一般的な要素表現へ。<br>
数値も、HeapNumberへの参照として扱われる場面が出てきます。
</div>
...
<div class="callout" style="margin-top:28px">
一度より一般的なElements Kindへ遷移すると、<br>
...
<div class="two-col wide">
  <div>
    <div class="col-label">PACKED</div>

```js
const xs = [1, 2, 3]
```

    <p class="lede">隙間がない。<br>V8が攻めた最適化をしやすい。</p>
  </div>
  <div>
    <div class="col-label">HOLEY</div>
...
```
    ...
    <p class="lede">穴がある。<br>追加チェックやプロトタイプ探索が必要になる。</p>
  </div>
</div>
...
PACKEDは隙間のない密な配列です。
HOLEYは穴がある疎な配列です。穴があると、単にundefinedを返せばいいとは限らず、プロトタイプチェーンの探索なども絡むので、最適化しにくくなります。
-->
...
xs.push(-0)       // PACKED_DOUBLE_ELEMENTS へ
xs.push(NaN)      // PACKED_DOUBLE_ELEMENTS へ
xs.push(Infinity) // PACKED_DOUBLE_ELEMENTS へ
```

<div class="callout" style="margin-top:28px">
見た目は「整数っぽい」値でも、<br>
Smiでは表現できないNumberはdouble扱いになります。
</div>
...
<code>PACKED_SMI_ELEMENTS</code> のようなElements Kindが表示されます。
</div>
...
<div class="callout" style="margin-top:28px">
Elements Kindが変わったタイミングをログで追えます。<br>
「配列の型が途中で変わる」瞬間が見えるようになります。
</div>
...
<div class="big-message">
  <span>V8のために</span><br>
  <strong>無理なマイクロ最適化はしない</strong>
</div>

<div class="callout" style="margin-top:36px">
V8はかなり賢いです。<br>
内部表現を知る目的は、普段のコードを全部Smiに寄せることではありません。
</div>
...
<div class="two-col wide">
  <div>
    <div class="col-label">避けたい揺れ</div>
    <ul class="clean">
      <li>数値配列に突然文字列を混ぜる</li>
      <li>密な配列を不用意に疎にする</li>
      <li>NaNやInfinityが大量に混ざる前提を放置する</li>
    </ul>
  </div>
  <div>
    <div class="col-label">安定しやすい形</div>
    <ul class="clean">
      <li>同じ意味の値は同じ表現に寄せる</li>
      <li>配列はできるだけ密に保つ</li>
      <li>数値大量処理ならTypedArrayも検討する</li>
    </ul>
  </div>
</div>
...
<ul class="takeaways">
  ...
  <li><strong>小さな整数はSmi</strong>、小数などはHeapNumberになりうる</li>
...
<div class="callout" style="margin-top:28px">
V8の内部実装はバージョンやビルド設定、アーキテクチャによって変わります。<br>
このスライドでは、理解しやすいように代表的な概念に絞って説明しています。
</div>
